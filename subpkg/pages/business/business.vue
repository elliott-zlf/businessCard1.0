<template>
  <view class="visit_box">
      ...
      <view class="business_data">
        ...
        <view class="data_content">
          <view class="seen_my" @click="handleHaveSeen('0')">
            <view class="seen_num">
              {{cardData[0]}}
              <view v-if="cardData[3]!==0" class="seen_badge">{{cardData[3]}}</view>
            </view>
          ...
          <view class="seen_my" @click="handleHaveSeen('1')">
            <view class="seen_num">
              {{cardData[1]}}
              <view v-if="cardData[4]!==0" class="seen_badge">{{cardData[4]}}</view>
            </view>
            <view class="seen_my_tile_box">
              <image
                class="seen_icon"
          ...
          <view class="seen_my" @click="handleHaveSeen('2')">
            <view class="seen_num">
               {{cardData[2]}}
               <view v-if="cardData[5]!==0" class="seen_badge">{{cardData[5]}}</view>
            </view>
            <view class="seen_my_tile_box">
              <image
                class="seen_icon"
    ...
    <view class="jisuangaodu">
        ...
        scroll-y
      >
          <readmy :dataList="readList"></readmy>
          <view v-if="readList.length !==0">
...
import { followData,changeFocus } from '@/api/carddetails.js'
    ...
    async handlegainCardData(item) {
     ...
     async reachBottom() {
      if (this.has_next) {
				...
				// })
				const res = await followData(this.pageData)
