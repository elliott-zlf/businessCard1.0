<template>
  <view class="container">
    ...
    <view class="content">
        ...
        <view class="popup_conent">
          ...
          <view>
            ...
            <view class="check_WeChat_tips"
              style="margin-bottom: 76.232rpx;"
              >资料完整度不够，<br/> 请先创建名片并上传作品</view
            >
      ...
      <!-- 上传作品弹框 -->
      <view class="pys_popup" v-if="uploadShow">
        <view class="popup_conent">
          <image
            class="closeicon"
            src="@/static/home/close.png"
          ...
          <view>
            ...
            <view class="check_WeChat_tips" style="margin-bottom: 35px"
              >请先将试音MP3文件发送到微信中，<br/> 小程序仅支持从聊天文件上传</view
            >
            <view class="share_btn_box">
              <button
                class="cancelSharebtn"
              ...
              <button class="sharebtn" @click="onUpload('上传音频')">
                立即上传
              </button>
            </view>
          </view>
        ...
        >
          <view class="order_list">
            ...
            <view class="order-content">
              ...
              <view class="order-label_item">
                <view class="order_label"></view>
                <view class="order_download">
                  <view
                    class="download_box"
                    @click="
                      downloadcopy(orderData.word_url, '下载链接已复制到粘贴板，请发送至电脑上点击下载')
                    "
                  >
                    <image
                      class="download_icon"
                    ...
                    <view class="download_text">{{
                      orderData.word_url ? "下载文本附件" : "无文本附件"
                    }}</view>
                  </view>
                  <view
                    class="download_box"
                    @click="
                      downloadcopy(orderData.work_url, '下载链接已复制到粘贴板，请发送至电脑上点击下载')
                    "
                  >
                    <image
                      class="download_icon"
                    ...
                    />
                    <view class="download_text">{{
                      orderData.work_url ? "下载参考样音" : "无参考样音"
                    }}</view>
                  </view>
                </view>
              </view>
            ...
            <view class="countdown_num_box">
              <!-- <view>
                ...
                <u-count-down class="countdown_num" :timestamp="186400" font-size = '32.609' separator-size="25.362" separator-color="#FFA006" separator="zh" color='#FFA006'></u-count-down>
              </view> -->
              <view class="countdown_num_box">
            ...
            <view class="delivery_btn_box" v-if="orderData.status === 0">
              <view v-if="orderData.cate===0">
                <view
                  v-if="orderData.type === 0"
                  class="delivery_btn"
                  @click.stop="handleAudition"
                  >我要试音</view
                >
                <view v-if="orderData.type === 1" class="end_btn">我已试音</view>
              </view>
              <view v-if="orderData.cate===1">
                <view class="delivery_btn" @click="handlebuEndorderShow(orderData.id)">提前结束需求</view>
              </view>
            </view>
          ...
          <view class="divider">
            ...
            <view class="text"
              >{{ orderData.teacher_count || 0 }}人已试音</view
            >
            <view class="line"></view>
          </view>
          <view
          ...
          >
            <image
              v-if="orderItem.status === 1 && orderData.cate != 1"
              class="zhongbiao"
              src="@/static/home2/zhongbiao.png"
              mode="scaleToFill"
            />
            <view
              class="teacher_top_Introduction"
              @click="handleVisitorsJump(orderItem)"
            >
              <view class="teacher_top_Introduc">
                ...
                <view class="top_Introduction_contenr">
                  ...
                  <view class="createtime">{{orderItem.createtime}}</view>
                </view>
                <view class="top_Introduction_right">
									<view v-if="orderData.cate===1" :class="[orderItem.status === 0 ? 'winning':'nowinning']" @click.stop="handleorderBid(orderItem.id)">
                    {{orderItem.status === 0 ? '操作中标':'已中标'}}
                  </view>
								</view>
              </view>
            </view>
            <view class="teacher_bot_voices">
              <view
                class="teacher_play_box"
                @click="playTheMusic(orderItem.teacher)"
              >
                <image
                  class="teacher_play_icon"
                  :src="orderItem.teacher.works.playStatus ? playActive : play"
                  mode="scaleToFill"
                />
                <view class="service_name u-line-1">
                  {{ orderItem.teacher.works.title }}
                </view>
              </view>
              <view
                class="download_iconbox"
                @click="
                  downloadcopy(
                    orderItem.teacher.works.url,
                    '下载链接已复制到粘贴板，请发送至电脑上点击下载'
                  )
                "
              >
                <image
          ...
          <view style="height: 326.087rpx"></view>
        </scroll-view>
      </view>
    </view>
...
export default {
  ...
  methods: {
    ...
    handleAudition() {
      profileStatus()
        .then((res) => {
          console.log("可以上传", res);
          this.uploadShow = true;
        })
        .catch((err) => {
          this.notputShow = true;
          console.log("还没有资质", err);
        });
    },
    // 操作中标
    handleorderBid(id) {
    ...
    onUpload(item) {
      ...
      this.$refs.lFile.upload({
        ...
        // #endif
        url: "https://www.peiyinstreet.com/business/chat/order", //替换为你的
        name: "file",
        header: {
          //根据你接口需求自定义
    ...
    onSuccess(res) {
      if (this.forUploading === "上传音频") {
        orderdelivery({ id: this.orderId, work_id: res.data.data })
          .then((res) => {
            uni.showToast({
              title: "上传成功，发布者选中后会主动联系您",
              icon: "none",
              duration: 3000,
            });
            this.getOrderdetails();
...
.content {
  ...
  .order_list {
    ...
    .bottom_line {
      border-bottom: #f6f7fa solid 1.812rpx;
      margin: 32.609rpx 0;
    }
    .countdown_num_box {
      width: 100%;
    ...
    .delivery_btn_box {
      ...
      .delivery_btn {
        ...
        height: 90.58rpx;
        background: #1274ff;
        box-shadow: 0rpx 14.493rpx 18.116rpx 0rpx rgba(18, 70, 255, 0.18);
