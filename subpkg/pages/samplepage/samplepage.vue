				...
				<view class="personalFile">
					...
					<view class="personal_list">
						...
						<view class="personal_content focusBox">
							...
							<view class="WeChat" v-if="false" @click="handleInviteDubbing">查看微信</view>
							<view class="WeChat" v-if="false" @click="downloadcopy(dataList.wechat_number ,'复制成功，添加时备注：名片')">复制微信</view>
							<view class="focus" v-if="dataList.follow===0" @click="handleFocus(dataList.id,0)">
								<image
									class="focusaixin"
							...
							<view v-if="dataList.follow===1" class="beenFocused" @click="handleFocus(dataList.id,1)">已关注</view>
						</view>
					</view>
				</view>
			</view>
