<template>
	<view class="container">
	  ...
	  <cardball v-if="userStatus<2" @handleChange="handleruku">
	  </cardball>
	  <cardball v-if="userStatus>=2" title="查看排名" @handleChange="handlerankingPopShow">
		  <image
		      slot="icon"
			  src="@/static/home2/rank.png"
			  mode="scaleToFill"
		  />
	  </cardball>
...
import playActive from '@/static/teacherlist/palyActive2.png'
	export default {
		...
		methods: {
			...
			handleGoBack() {
				console.log('跳转回配音师库')
				uni.switchTab({ url: '/pages/voicelibrary/voicelibrary' })
			},
			handleServicetag() {
			...
			handleFocus(item,index) {
				changeFocus({id:item.teacher_id,type: index}).then((res)=>{
